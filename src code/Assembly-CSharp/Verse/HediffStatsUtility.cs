...
namespace Verse
{
	...
	{
		// Token: 0x06000705 RID: 1797 RVA: 0x000254D9 File Offset: 0x000236D9
			...
			if (stage != null)
			{
				...
				if (stage.statOffsets != null)
				{
					...
					for (int i = 0; i < stage.statOffsets.Count; i = num3 + 1)
					{
						...
						{
							float num4 = statModifier.value;
							if (instance != null && instance.CurStage.multiplyStatChangesBySeverity)
							{
								num4 *= instance.Severity;
							}
							yield return new StatDrawEntry(StatCategoryDefOf.CapacityEffects, statModifier.stat.LabelCap, statModifier.stat.Worker.ValueToString(num4, false, ToStringNumberSense.Offset), statModifier.stat.description, 4070, null, null, false);
						}
						num3 = i;
					}
				}
				if (stage.statFactors != null)
				{
					...
					for (int i = 0; i < stage.statFactors.Count; i = num3 + 1)
					{
						...
						{
							float num5 = statModifier2.value;
							if (instance != null && instance.CurStage.multiplyStatChangesBySeverity)
							{
								num5 = StatWorker.ScaleFactor(num5, instance.Severity);
							}
							yield return new StatDrawEntry(StatCategoryDefOf.CapacityEffects, statModifier2.stat.LabelCap, statModifier2.stat.Worker.ValueToString(num5, false, ToStringNumberSense.Factor), statModifier2.stat.description, 4070, null, null, false);
						}
						num3 = i;
					}
				}
